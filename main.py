...
def recommend_music(song_name, song_data_path, user_data_path, num_recommendations=10):
    ...
    if song_index_df.count() > 0:
        ...
        if recommendations:
            ...
            for rec in result_df.collect():
                print(
                    f"Recommended song ID: {rec.song_id}, Title: {rec.title}, Artist: {rec.artist_name}, Predicted listen count: {rec.rating:.2f}")
        else:
...
if __name__ == "__main__":
    ...
    print(f"\nSong Recommendation on Song: {song_name}")
    recommend_music(song_name, song_data_path, user_data_path, num_recommendations=10)